...
export function FlagGrid({ end, prideInfo, flippedArray }) {
  console.log("flagData", prideInfo);
  const [flagLoad, setFlagLoad] = useState(false);
  const flagImg = useMemo(() => {
    const img = new Image();
    img.onload = () => setFlagLoad(true);
    img.src = `https://pride.dev/api/flags/${prideInfo.id}/SVG`;
    return img;
  }, [prideInfo]);
